...
names <- c("Haldane - Beta(0, 0)","Flat -  Beta(1, 1)", "Non-informative - Beta(.5, .5)")
...
kable(results, caption="Normal Approx: Probability difference in logodds is greater than 0")
```
...
#kable(func_logdiff(it.vec, 0, 0), caption="Haldane Prior - Beta(0, 0)")
#kable(func_logdiff(it.vec, 1, 1), caption="Flat Prior -  Beta(1, 1)")
#kable(func_logdiff(it.vec, .5, .5), caption="Non-informative Prior - Beta(.5, .5)")

t.H <- func_logdiff(it.vec, 0, 0)
t.F <- func_logdiff(it.vec, 1, 1)
t.N <- func_logdiff(it.vec,.5, .5)

long_dt <- rbind(t.H, t.F, t.N)
