...
test <- fisher.test(blue, or=1, alternative="greater")
test
paste("Probabilitiy that log differences are greater than 0:", decimal(1 - test$p.value, dec))
```
...
alpha.count <- c(127, 123,  93, 17)
gamma.count <- c(345, 209, 158, 65)
parameters <- cbind(alpha.count, gamma.count)

it <- 5
func_delinq <- function(it){
set.seed(050104)
proportions <- rdirichlet(it, parameters)
oldest.diff <- proportions[,1] - proportions[,5] #most delinq. oldest - least delinq. oldest 
CI.val <- decimal(quantile(oldest.diff, c(0.025, 0.975)), dec)
CI.95 <- paste("(", paste(CI.val, collapse=", "), ")")
mean <- decimal(mean(oldest.diff), dec)
vec <- c(it, mean, CI.95)
vec}

it.vec <- c(1e04, 1e05, 1e06)
dt <- sapply(it.vec, func_delinq )
rownames(dt) <- c("Iterations", "Mean", "95% CI")
knitr::kable(dt, booktabs=T, 'latex', caption="Difference in Proportion Amoung Oldest: Most - Least") %>%
  kableExtra::kable_styling(latex_options="hold_position" ) #hold table in place 
...
\\
\sqrt{n} \left( g(\hat{p})- g(p) \right) & \stackrel{\mathcal{D}}{\to} \mathcal{N} \left( 0, \ \sigma^2 [ g'(p)]^2 \right) 
\\
...
\\[0.5ex]
\sqrt{n} \left( g(\hat{p})- g(p) \right) & \stackrel{\mathcal{D}}{\to} \mathcal{N} \left( 0, \ \tfrac{1}{n} \right) 
\\
g(\hat{p}) & \stackrel{\mathcal{D}}{\to} \mathcal{N} \left( g(\hat{p}), \ 1 \right)
\\
2\sin^{-1}\sqrt{\hat{p}} & \stackrel{\mathcal{D}}{\to} \mathcal{N} \left( 2\sin^{-1}\sqrt{p}\ ,\ 1 \right)
\end{aligned} 
$$
...
\text{Let } h(x, y) & =  x / y \\ 
\text{Let } h(B) & = \overline{x} / \overline{y} \\
...
\stackrel{\mathcal{D}}{\to} \mathcal{N} \left( 0, \frac{1}{n} \left( \frac{ 1}{\phi^2} - \frac{ \sigma^2}{\phi^4} \right) \right) 
\\[1ex]
\frac{ \overline{x}}{\overline{y}} & \stackrel{\mathcal{D}}{\to}
\mathcal{N} \left( 
\frac{\theta}{\phi}, \ 
\frac{ 1}{\phi^2} - \frac{ \sigma^2}{\phi^4}
\right)
\end{aligned} 
$$
