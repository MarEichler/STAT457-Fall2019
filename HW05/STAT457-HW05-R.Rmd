...
\newpage  
### Problem 1d  
Is deliquency releated to birth order? 
```{r 1d-counts}
...
table.count <- decimal(cbind(parameters, total), dec)
rownames(table.count) <- c("Most Delinquent", "Least Delinquent")
colnames(table.count) <- c("Oldest", "In-between", "Youngest", "Only Child", "Total")
knitr::kable(table.count, booktabs=TRUE, 'latex', caption="Row Proportion") %>%
...
There is evidence that birth order has an effect on deliquency rates.  
