...
```{r p4a-Uniform, warning=FALSE, message=FALSE}
func_J.Uniform <- function(it, b, f){
x1 <- runif(it, b, f)
J1 <- sum((func_Like.Y(x1) / x1)/sum(x1))
return(c(J1))
}
...
it.vec <- c(1e04, 1e05, 1e06)
set.seed(0604011)
IS.Uniform.J1 <- mapply(func_J.Uniform, it.vec, b, f)

Uniform.df <- func_big.df.Uniform(it.vec, b, f)
Uniform.text.df <- data.frame(label =c(
                paste("J1=", round(IS.Uniform.J1[1], 10)),
                paste("J1=", round(IS.Uniform.J1[2], 10)),
                paste("J1=", round(IS.Uniform.J1[3], 10))
                ),
                       Iteration = it.vec)
...
```{r p4a-Beta, warning=FALSE, message=FALSE}
...
it.vec <- c(1e04, 1e05, 1e06)
set.seed(0604012)
IS.Beta.J1 <- mapply(func_J.Beta, it.vec, b, f)

Beta.df <- func_big.df.Beta(it.vec, b, f)
Beta.text.df <- data.frame(label =c(
                paste("J1=", round(IS.Beta.J1[1], 10)),
                paste("J1=", round(IS.Beta.J1[2], 10)),
                paste("J1=", round(IS.Beta.J1[3], 10))
                ),
                       Iteration = it.vec)
func_plots(Beta.df, Beta.text.df, "Beta")
```

```{r p4a-Normal, warning=FALSE, message=FALSE}
func_J.Normal <- function(it, b, f){
alpha <- b + 1 
beta <- f + 1
mean <- alpha / (alpha + beta)
var <- (alpha*beta) / ( (alpha + beta)^2 * (alpha + beta + 1) )
x1 <- rnorm(it, mean, sqrt(var))
J1 <- sum((func_Like.Y(x1) / x1)/sum(x1))
return(c(J1))
}
...
it.vec <- c(1e04, 1e05, 1e06)
set.seed(0604012)
IS.Normal.J1 <- mapply(func_J.Normal, it.vec, b, f)

Normal.df <- func_big.df.Normal(it.vec, b, f)
Beta.text.df <- data.frame(label =c(
                paste("J1=", round(IS.Normal.J1[1], 10)),
                paste("J1=", round(IS.Normal.J1[2], 10)),
                paste("J1=", round(IS.Normal.J1[3], 10))
                ),
                       Iteration = it.vec)
func_plots(Beta.df, Beta.text.df, "Normal")
```

$\text{\textcolor{red}{4A:OUTSTANDING}}$ 

### Problem 4b  
Repeat the calculation using numerical integration.  Compare the results of (a) and (b).  

```{r p4b}
Integrate.J1 <- integrate(func_Like.Y, lower=b, upper=f)$value

iteration <- c("N/A", it.vec)
J1.vec <- c(Integrate.J1, IS.Uniform.J1, IS.Beta.J1)
Integrate.J1.vec <- rep(Integrate.J1, length(J1.vec))
diff.J1 <- J1.vec - Integrate.J1.vec

result.4b <- rbind(iteration, round(J1.vec, 6), round(diff.J1, 6))
rownames(result.4b) <- c("It", "J1", "J1-Intg")
result.4b
```
```{r p4b-table}
knitr::kable(result.4b, booktabs=T, 'latex') %>%
  kableExtra::kable_styling(latex_options="hold_position" ) %>% #hold table in place 
  kableExtra::add_header_above(c(" "=1, "Integration"=1, "IS - Uniform"=3, "IS - Beta"=3)) #need to have a space in empty columns 
```



The standard error is pretty high and the estimates differ by about 0.003.
```{r Mena}
u<-runif(10000,0.7,0.9)
I<-1/(0.9-0.7)
su<-sum(u)
j<-sum((1/u*u*(1-u)^4)/su)
j 
sd<-sqrt(sum(1/5*(u-j)^2))
sd
k<-function(x){
  x*(1-x)^4
}
integrate(k, lower=0.7,upper=0.9)
```

$\text{\textcolor{red}{4B:OUTSTANDING}}$ 
