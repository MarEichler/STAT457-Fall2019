...
func_Metropolis <- function(startseed, m, Y, driver){
  set.seed(startseed)
  theta.i <- runif(1)
  chain <- c()
  alpha <- c()
...
func_driver2.Norm.sd.01 <- function(n){rnorm(n, mean=theta.i, sd=0.01)}
func_driver3.Norm.sd.1 <-  function(n){rnorm(n, mean=theta.i, sd=0.1)}
func_driver4.Norm.sd.5 <-  function(n){rnorm(n, mean=theta.i, sd=0.5)}
func_driver5.Norm.mu.4sd.5 <-  function(n){rnorm(n, mean=0.4, sd=0.5)}
```

```{r p2.func_p2ABC}
func_problem2ABC <- function(startseed, m, Y, driver, driver.name){
#main title
print.Y <- paste(Y, collapse=",")
main <- paste("Metropolis for data Y=(", print.Y, ") using the driver", driver.name)

#get chain and make dataframe 
chain <- func_Metropolis(startseed, m, Y, driver)
df <- data.frame(
         "theta.i"=chain
        ,"i"=c(1:length(chain))
)
...
```{r p2a, fig.height=2, eval=FALSE}
Y.A <- c(125, 18, 20, 34)

func_problem2ABC(211, 10000, Y.A, func_driver1.Uniform, "Uniform(0,1)")
func_problem2ABC(212, 10000, Y.A, func_driver2.Norm.sd.01,   "Normal(theta.i, 0.01)")
func_problem2ABC(213, 10000, Y.A, func_driver3.Norm.sd.1,    "Normal(theta.i, 0.10)")
func_problem2ABC(214, 10000, Y.A, func_driver4.Norm.sd.5,    "Normal(theta.i, 0.50)")
func_problem2ABC(215, 10000, Y.A, func_driver5.Norm.mu.4sd.5,"Normal(0.40, 0.10)")
```

\newpage  
## Problem 2b  
Repeat 2a for $Y = (14, 0, 1, 5)$  
```{r p2b, fig.height=2, eval=FALSE}
Y.B <- c(14, 0, 1, 5)

func_problem2ABC(221, 10000, Y.B, func_driver1.Uniform, "Uniform(0,1)")
func_problem2ABC(222, 10000, Y.B, func_driver2.Norm.sd.01,   "Normal(theta.i, 0.01)")
func_problem2ABC(223, 10000, Y.B, func_driver3.Norm.sd.1,    "Normal(theta.i, 0.10)")
func_problem2ABC(224, 10000, Y.B, func_driver4.Norm.sd.5,    "Normal(theta.i, 0.50)")
func_problem2ABC(225, 10000, Y.B, func_driver5.Norm.mu.4sd.5,"Normal(0.40, 0.10)")
```
...
```{r p2.func_Metroinfo}
func_Metroinfo <- function(startseed, m, Y, driver){
chain <- func_Metropolis(startseed, m, Y, driver)
it <- length(chain)
chain.mu <- mean(chain)
chain.sd <- sd(chain)
...
```{r p2.func_20chains}
func_20metrochains <- function(Y, driver, start){
print.Y <- paste(Y, collapse=",")
cnames <- c("Mean", "SD", "SE")
info.20chain <- mapply( func_Metroinfo
                       , startseed=c(start:(start+19))
                       , m =rep(10000, 20)
                       , Y=rep(list(Y), 20)
                       , driver=rep(list(func_driver1.Uniform), 20)
                      )
info.20chain <- t(info.20chain)
colnames(info.20chain) <- cnames
info.20chain
...
d1 <- func_20metrochains(Y.D, func_driver1.Uniform, 10)
d2 <- func_20metrochains(Y.D, func_driver2.Norm.sd.01, 20)
d3 <- func_20metrochains(Y.D, func_driver3.Norm.sd.1, 30)
d4 <- func_20metrochains(Y.D, func_driver4.Norm.sd.5, 40)
d5 <- func_20metrochains(Y.D, func_driver5.Norm.mu.4sd.5, 50)
