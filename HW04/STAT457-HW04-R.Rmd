...
func_dpost3b <- function(x){dunif(x, .19, .35)*dbeta(x, shape1=a, shape2=b)}
func_ppost3b <- function(x){punif(x, .19, .35)*pbeta(x, shape1=a, shape2=b)}
...
set.seed(040302)
x.sim <- seq(.19, .35, length=1000)
y.sim <- func_post3b(x.sim)
df <- data.frame("Xsim" = x.sim, "Ysim"=y.sim)
x.max <- df$Xsim[Ysim=max(y.sim)]
...
val <- length(data.sim[data.sim >= 0.200]) / length(data.sim) 
CI.low <-quantile(data.sim, .025)
CI.up <- quantile(data.sim, .975)
#df <- data.frame("x"=x, "y"=y)
...
plot2 <- ggplot(df, aes(Xsim, Ysim))+ geom_line(lwd = 1.5, linetype="dotted", aes(col=dist[2]))+
...
x.sim <- c(1, 1, 1, 1)
y.sim <- c(0, .5, .5,0)
times <- round(y.sim*it)
for (i in 1:it){
  if (times[i] > 0 ) {
    new.data <- rep(x.sim[i], times[i])
    sim.data <- c(sim.data, new.data)
  } else {sim.data <- sim.data}
  }
...
a0 <- .5
b0 <- .5
func_distplot3c(Binomial.Likelihood, a0, b0, "Jeffery's Prior: Beta(1, 1)")
```

```{r, echo=FALSE, warning=FALSE, fig.width=10, fig.height=4}
a0 <- 0
b0 <- 0
func_distplot3c(Binomial.Likelihood, a0, b0, "Haldane's Prior: Beta(1, 1)")
```
